...
void AuNaPurePursuitController::configure(
...
void AuNaPurePursuitController::cleanup() {
  RCLCPP_INFO(logger_,
              ...
              " auna_pure_pursuit_controller::AuNaPurePursuitController",
              plugin_name_.c_str());
...
AuNaPurePursuitController::createCarrotMsg(
...
double AuNaPurePursuitController::getLookAheadDistance(
...
AuNaPurePursuitController::computeVelocityCommands(
  ...
  if (shouldRotateToGoalHeading(carrot_pose)) {
    ...
    applyConstraints(regulation_curvature, speed,
  ...
  if (params_->use_collision_detection &&
    ...
    throw nav2_core::PlannerException(
        "AuNaPurePursuitController detected collision ahead!");
  }
...
bool AuNaPurePursuitController::shouldRotateToPath(
...
bool AuNaPurePursuitController::shouldRotateToGoalHeading(
    const geometry_msgs::msg::PoseStamped& carrot_pose) {
...
void AuNaPurePursuitController::rotateToHeading(
...
geometry_msgs::msg::Point AuNaPurePursuitController::circleSegmentIntersection(
...
geometry_msgs::msg::PoseStamped AuNaPurePursuitController::getLookAheadPoint(
  ...
  } else if (params_->use_interpolation &&
...
void AuNaPurePursuitController::applyConstraints(
  ...
  if (params_->use_cost_regulated_linear_velocity_scaling) {
    cost_vel = heuristics::costConstraint(linear_vel, pose_cost, costmap_ros_,
                                          params_);
  }
...
void AuNaPurePursuitController::setSpeedLimit(const double& speed_limit,
...
double AuNaPurePursuitController::findVelocitySignChange(
  ...
  for (unsigned int pose_id = 1; pose_id < transformed_plan.poses.size() - 1;
    ...
    if ((oa_x * ab_x) + (oa_y * ab_y) < 0.0) {
      ...
      return hypot(transformed_plan.poses[pose_id].pose.position.x,
                   transformed_plan.poses[pose_id].pose.position.y);
    }
  }
...
PLUGINLIB_EXPORT_CLASS(auna_pure_pursuit_controller::AuNaPurePursuitController,
