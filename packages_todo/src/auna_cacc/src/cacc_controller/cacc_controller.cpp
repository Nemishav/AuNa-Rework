...
CaccController::CaccController() : Node("cacc_controller") {
  sub_cam_ = this->create_subscription<auna_its_msgs::msg::CAM>(
      "cam_filtered", 2, [this](const auna_its_msgs::msg::CAM::SharedPtr msg) {
  ...
  pub_cam_debug_ =
      this->create_publisher<auna_its_msgs::msg::CAM>("cacc/cam_debug", 1);
  ...
  params_.curvature_lookahead =
      this->get_parameter("curvature_lookahead").as_int();
...
void CaccController::cam_callback(
    const auna_its_msgs::msg::CAM::SharedPtr msg) {
  cam_x_ = msg->x;
  cam_y_ = msg->y;
  cam_velocity_ = msg->v;
  ...
  if (last_cam_msg_ == nullptr) {
    ...
    double dt =
        msg->header.stamp.sec - last_cam_msg_->header.stamp.sec +
...
void CaccController::odom_callback(
  ...
  odom_velocity_ = sqrt(pow(msg->twist.twist.linear.x, 2) +
...
void CaccController::pose_callback(
...
void CaccController::timer_callback() {
  if (params_.use_waypoints) {
    if (auto_mode_) {
      ...
      // Calculate target waypoint index (the waypoint that is closest to the
      // vehicle and within the time gap

      double target_distance = params_.standstill_distance +
                               params_.time_gap * params_.target_velocity;
      ...
      int next_index = (target_waypoint_index_ + params_.curvature_lookahead) %
                       num_waypoints;
      ...
      double dx = waypoints_x_[(curr_index + params_.curvature_lookahead) %
                               num_waypoints] -
                  waypoints_x_[curr_index];
      double dy = waypoints_y_[(curr_index + params_.curvature_lookahead) %
                               num_waypoints] -
      ...
      // scale the cam_velocity_ between target_velocity and target_velocity/2,
      // inverse proportional to cam_yaw_rate_, whose values are between 0
      // and 1.25

      cam_velocity_ = params_.target_velocity -
                      (params_.target_velocity / 2) * (cam_yaw_rate_ / 1.25);
      ...
      // Find the previous index at which the distance to the closest waypoint
      // is at least params_.extra_distance
      for (int i = closest_waypoint_index; true;
           i = (i - 1 + num_waypoints) % num_waypoints) {
        double dx = waypoints_x_[i] - waypoints_x_[closest_waypoint_index];
        double dy = waypoints_y_[i] - waypoints_y_[closest_waypoint_index];
        double distance_squared = dx * dx + dy * dy;

        if (distance_squared >=
            params_.extra_distance * params_.extra_distance) {
          // Stop the loop if the distance is increasing
          break;
        }
      ...
      // yaw difference modulo for the case that one is negative and the other
      // positive
      double yaw_difference;
      if (next_yaw - current_yaw > M_PI) {
        yaw_difference = next_yaw - current_yaw - 2 * M_PI;
      ...
      // Calculate the required time to reach the n-th next waypoint
      double dx =
          waypoints_x_[(target_waypoint_index_ + params_.curvature_lookahead) %
                       num_waypoints] -
          waypoints_x_[target_waypoint_index_];
      double dy =
          waypoints_y_[(target_waypoint_index_ + params_.curvature_lookahead) %
                       num_waypoints] -
  ...
  if (cam_curvature_ <= 0.01 && cam_curvature_ >= -0.01) {
    s_ =
        0.5 *
            pow(params_.standstill_distance + params_.time_gap * odom_velocity_,
                2) *
            cam_curvature_ -
        0.125 *
            pow(params_.standstill_distance + params_.time_gap * odom_velocity_,
    ...
    s_ = (-1 + sqrt(1 + pow(cam_curvature_, 2) *
                            pow(params_.standstill_distance +
                                    params_.time_gap * odom_velocity_,
                                2))) /
         cam_curvature_;
  }
  alpha_ = atan(cam_curvature_ * (params_.standstill_distance +
                                  params_.time_gap * odom_velocity_));
  ...
  z_2_ = cam_y_ - pose_y_ - s_ * cos(cam_yaw_) -
         (params_.standstill_distance + params_.time_gap * odom_velocity_) *
             sin(pose_yaw_);
  z_3_ =
      cam_velocity_ * cos(cam_yaw_) - odom_velocity_ * cos(pose_yaw_ + alpha_);
  z_4_ =
      cam_velocity_ * sin(cam_yaw_) - odom_velocity_ * sin(pose_yaw_ + alpha_);

  invGam_Det_ =
      (params_.standstill_distance + params_.time_gap * odom_velocity_) *
      (params_.time_gap -
       params_.time_gap * sin(s_) * sin(cam_yaw_ - pose_yaw_));
  ...
  invGam_2_ =
      ((params_.standstill_distance + params_.time_gap * odom_velocity_) *
       sin(pose_yaw_)) /
      invGam_Det_;
  invGam_3_ = (-params_.time_gap * sin(pose_yaw_) -
               params_.time_gap * sin(s_) * cos(cam_yaw_)) /
              invGam_Det_;
  invGam_4_ = (params_.time_gap * cos(pose_yaw_) -
               params_.time_gap * sin(s_) * sin(cam_yaw_)) /
              invGam_Det_;
...
void CaccController::set_target_velocity(
  ...
  RCLCPP_INFO(this->get_logger(), "Target velocity is now %f",
...
void CaccController::set_extra_distance(
    const std::shared_ptr<auna_msgs::srv::SetFloat64::Request> request,
    std::shared_ptr<auna_msgs::srv::SetFloat64::Response> response) {
  RCLCPP_INFO(this->get_logger(), "Setting extra distance to %f",
              request->value);
  // if the request value is 1.0, increase the parameter by 0.5, if it is 0.0,
  // decrease it by 0.5
  if (request->value == 1.0) {
  ...
  RCLCPP_INFO(this->get_logger(), "Extra distance is now %f",
...
void CaccController::set_standstill_distance(
    const std::shared_ptr<auna_msgs::srv::SetFloat64::Request> request,
    std::shared_ptr<auna_msgs::srv::SetFloat64::Response> response) {
  RCLCPP_INFO(this->get_logger(), "Setting standstill distance to %f",
              request->value);
...
CaccController::dynamicParametersCallback(
  ...
  for (auto parameter : parameters) {
    ...
    // print
    RCLCPP_INFO(this->get_logger(), "Parameter '%s' was changed.",
                name.c_str());
