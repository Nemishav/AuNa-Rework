...
CamCommunication::CamCommunication() : Node("cam_communication") {
  ...
  cam_filtered_publisher_ =
      this->create_publisher<auna_its_msgs::msg::CAM>("cam_filtered", 2);
  cam_publisher_ = this->create_publisher<auna_its_msgs::msg::CAM>("/cam", 2);
  cam_subscriber_ = this->create_subscription<auna_its_msgs::msg::CAM>(
      "/cam", 2, [this](auna_its_msgs::msg::CAM::SharedPtr msg) -> void {
...
void CamCommunication::timer_callback() {
  if (this->now() - last_cam_msg_time_ >= std::chrono::milliseconds(1000)) {
    ...
    } else if (sqrt(pow(this->longitude_ - last_cam_msg_.x, 2) +
                    pow(this->latitude_ - last_cam_msg_.y, 2)) > 0.4) {
...
void CamCommunication::odom_callback(
  ...
  this->speed_ = sqrt(pow(msg->twist.twist.linear.x, 2) +
                      pow(msg->twist.twist.linear.y, 2));

  if (last_odom_msg_ != nullptr) {
    this->old_speed_ = sqrt(pow(last_odom_msg_->twist.twist.linear.x, 2) +
                            pow(last_odom_msg_->twist.twist.linear.y,
