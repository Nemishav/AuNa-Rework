...
void CommunicationCostmap::onInitialize() {
  ...
  RCLCPP_INFO(node->get_logger(), "CommunicationCostmap: robot_index: %d",
              robot_index_);

  cam_sub_ = node->create_subscription<auna_its_msgs::msg::CAM>(
      "/cam", rclcpp::QoS(1),
      [this](const auna_its_msgs::msg::CAM::SharedPtr msg) {
        cam_callback(msg);
      });
...
void CommunicationCostmap::updateBounds(double /*robot_x*/, double /*robot_y*/,
...
void CommunicationCostmap::onFootprintChanged() {
  need_recalculation_ = true;

  RCLCPP_DEBUG(
      rclcpp::get_logger("nav2_costmap_2d"),
      "CommunicationCostmap::onFootprintChanged(): num footprint points: %lu",
...
// layers.
void CommunicationCostmap::updateCosts(nav2_costmap_2d::Costmap2D& master_grid,
  ...
  for (int j = min_j; j < max_j; j++) {
    for (int i = min_i; i < max_i; i++) {
      ...
      //       // Object's velocity is less than 0.1m, mark the cell as an
      //       obstacle cost = LETHAL_OBSTACLE; break; // No need to check other
      //       objects
      //     }
      //   }
      // }
...
void CommunicationCostmap::cam_callback(
