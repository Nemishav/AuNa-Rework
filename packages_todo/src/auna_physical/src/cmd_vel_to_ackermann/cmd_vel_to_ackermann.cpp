...
CmdVelToAckermann::CmdVelToAckermann() : Node("cmd_vel_to_ackermann_node") {
  cmd_vel_subscription = this->create_subscription<geometry_msgs::msg::Twist>(
      "cmd_vel", 2, [this](const geometry_msgs::msg::Twist::SharedPtr msg) {
  ...
  this->get_parameter<bool>("convert_yaw_to_steering_angle",
                            convert_yaw_to_steering_angle_);
  this->get_parameter<double>("wheelbase", wheelbase_);

  emergency_stop_active_ = false;

  // service server for emergency stop
  emergency_stop_service = this->create_service<std_srvs::srv::Empty>(
      ...
      [this](const std_srvs::srv::Empty::Request::SharedPtr request,
        ...
        emergency_stop_callback(request, response);
      });

  // service server for emergency stop disable
  emergency_stop_disable_service = this->create_service<std_srvs::srv::Empty>(
      "/emergency_stop_disable",
      [this](const std_srvs::srv::Empty::Request::SharedPtr request,
             const std_srvs::srv::Empty::Response::SharedPtr response) {
        emergency_stop_disable_callback(request, response);
      });
}

// Emergency stop callback
void CmdVelToAckermann::emergency_stop_callback(
...
void CmdVelToAckermann::emergency_stop_disable_callback(
    const std_srvs::srv::Empty::Request::SharedPtr request,
    const std_srvs::srv::Empty::Response::SharedPtr response) {
...
void CmdVelToAckermann::cmd_vel_callback(
  ...
  if (convert_yaw_to_steering_angle_) {
    if (fabs(msg->linear.x) < 0.001) {
      ...
      // use ackermann_msg.drive.steering_angle = atan(wheelbase_ *
      // msg->angular.z / msg->linear.x); to calculate the steering angle for
      // positive velocies, but swap sign for negative velocities
      ackermann_msg.drive.steering_angle =
          atan(wheelbase_ * msg->angular.z / fabs(msg->linear.x));
    }
  } else {
