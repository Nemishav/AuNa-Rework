...
ViconTFConverter::ViconTFConverter(std::string name)
    : Node("vicon_tf_converter"),
      buffer_(this->get_clock()),
      listener_(buffer_),
      broadcaster_(this) {
  // vicon_sub for vicon_callback using lambda function
  vicon_sub_ = this->create_subscription<geometry_msgs::msg::TransformStamped>(
      "vicon/pose", 2,
      [this](const geometry_msgs::msg::TransformStamped::SharedPtr msg)
          -> void { vicon_callback(msg); });
...
void ViconTFConverter::vicon_callback(
  ...
  // tf2::Quaternion q_vicon(msg->transform.rotation.x,
  // msg->transform.rotation.y, msg->transform.rotation.z,
  // msg->transform.rotation.w); tf2::Transform map_to_base(q_vicon,
  ...
  //     {
  //         odom_to_base_link_lookup =
  //         buffer_.lookupTransform(name_+"/odom",name_+"/base_link",
  //         tf2::TimePointZero);
  //     }
  // }
  ...
  //     return;
  // }
  // tf2::Quaternion q_ob(odom_to_base_link_lookup.transform.rotation.x,
  // odom_to_base_link_lookup.transform.rotation.y,
  // odom_to_base_link_lookup.transform.rotation.z,
  // odom_to_base_link_lookup.transform.rotation.w); tf2::Transform
  // odom_to_base(q_ob,
  // tf2::Vector3(odom_to_base_link_lookup.transform.translation.x,
  ...
  // map_to_odom_transform_msg.transform.translation.x =
  // map_to_odom.getOrigin()[0];
  // map_to_odom_transform_msg.transform.translation.y =
  // map_to_odom.getOrigin()[1];
  // map_to_odom_transform_msg.transform.translation.z =
  // map_to_odom.getOrigin()[2]; map_to_odom_transform_msg.transform.rotation.x
  // = map_to_odom.getRotation().getX();
  // map_to_odom_transform_msg.transform.rotation.y =
  // map_to_odom.getRotation().getY();
  // map_to_odom_transform_msg.transform.rotation.z =
  // map_to_odom.getRotation().getZ();
  // map_to_odom_transform_msg.transform.rotation.w =
  // map_to_odom.getRotation().getW(); map_to_odom_transform_msg.header.frame_id
  // = "map"; if (name_ == "")
  // {
  ...
  // tf2::durationFromSec(0.5); map_to_odom_transform_msg.header.stamp =
  // tf2_ros::toMsg(stamp);
  ...
  auto stamp_reference =
      tf2_ros::fromMsg(msg->header.stamp) + tf2::durationFromSec(0.5);
