...
ControlPanel::ControlPanel(QWidget* parent) : rviz_common::Panel(parent) {
  ...
  connect(changeVelocityButton, &QPushButton::clicked, this,
          &ControlPanel::onChangeVelocityClicked);
...
void ControlPanel::onEmergencyStopClicked() {
  ...
  auto client =
      node_->create_client<std_srvs::srv::Empty>("/emergency_stop_enable");
...
void ControlPanel::onChangeVelocityClicked() {
  // Get the desired velocity from the user input (assuming QLineEdit named
  // 'velocity_input_')
