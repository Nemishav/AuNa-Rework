...
class G29ForceFeedback : public rclcpp::Node {
 ...
 private:
  void targetCallback(
      const ros_g29_force_feedback::msg::ForceFeedback::SharedPtr in_target);
  void loop();
  int testBit(int bit, unsigned char *array);
  void initDevice();
  void calcRotateForce(double &torque, double &attack_length,
                       const ros_g29_force_feedback::msg::ForceFeedback &target,
                       const double &current_position);
  void calcCenteringForce(
      double &torque, const ros_g29_force_feedback::msg::ForceFeedback &target,
      const double &current_position);
  void uploadForce(const double &position, const double &force,
                   const double &attack_length);
};

G29ForceFeedback::G29ForceFeedback() : Node("g29_force_feedback") {
  sub_target =
      this->create_subscription<ros_g29_force_feedback::msg::ForceFeedback>(
          "/ff_target", rclcpp::SystemDefaultsQoS(),
          std::bind(&G29ForceFeedback::targetCallback, this,
                    std::placeholders::_1));
  ...
  declare_parameter("auto_centering_max_position",
                    m_auto_centering_max_position);
  ...
  timer = this->create_wall_timer(
      std::chrono::milliseconds((int)m_loop_rate * 1000),
...
void G29ForceFeedback::loop() {
  ...
  while (read(m_device_handle, &event, sizeof(event)) == sizeof(event)) {
    if (event.type == EV_ABS && event.code == m_axis_code) {
      m_position = (event.value - (m_axis_max + m_axis_min) * 0.5) * 2 /
                   (m_axis_max - m_axis_min);
    }
  }
...
void G29ForceFeedback::calcRotateForce(
...
void G29ForceFeedback::calcCenteringForce(
    double &torque, const ros_g29_force_feedback::msg::ForceFeedback &target,
    const double &current_position) {
  double diff = target.position - current_position;
  double direction = (diff > 0.0) ? 1.0 : -1.0;
  ...
  if (fabs(diff) < m_eps)
    ...
    double power =
        (fabs(diff) - m_eps) / (m_auto_centering_max_position - m_eps);
...
void G29ForceFeedback::uploadForce(const double &position, const double &torque,
                                   const double &attack_length) {
...
void G29ForceFeedback::targetCallback(
    const ros_g29_force_feedback::msg::ForceFeedback::SharedPtr in_msg) {
  if (m_target.position == in_msg->position &&
      m_target.torque == fabs(in_msg->torque)) {
    m_is_target_updated = false;
...
void G29ForceFeedback::initDevice() {
  ...
  if (ioctl(m_device_handle, EVIOCGBIT(EV_ABS, sizeof(abs_bits)), abs_bits) <
      0) {
