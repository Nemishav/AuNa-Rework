...
// and listener.
LocalizationPose::LocalizationPose(std::string prefix)
...
void LocalizationPose::timer_callback() {
  ...
  try {
    transformStamped = this->buffer.lookupTransform(
        prefix + "odom", prefix + "base_link", tf2::TimePointZero);
    transformStamped = this->buffer.lookupTransform("map", prefix + "base_link",
                                                    tf2::TimePointZero);
