...
SimulationPose::SimulationPose(std::string name) : Node("model_state_node") {
  publisher_ =
      this->create_publisher<auna_its_msgs::msg::CAM>("simulation_pose", 2);
...
void SimulationPose::service_timer_callback() {
  ...
  auto result = modelClient_->async_send_request(
      request, std::bind(&SimulationPose::model_srv_callback, this,
                         std::placeholders::_1));
}

// Read the requested entity state and publish the received pose to
// simulation_pose
void SimulationPose::model_srv_callback(
  ...
  simulation_pose.theta =
      yaw * 180 / M_PI -
      360 * floor(yaw * 180 / M_PI / 360);  // get heading in radians //1 degree
  simulation_pose.thetadot =
      entity->state.twist.angular.z * 180 / M_PI;  // 1 degree/s
  ...
  int drive_direction = simulation_pose.theta - velocity_heading > 90 ||
                        simulation_pose.theta - velocity_heading < 270;
  this->speed_ = sqrt(pow(entity->state.twist.linear.x, 2) +
                      pow(entity->state.twist.linear.y, 2)) *
                 scale_factor_;  // 1 m/s

  simulation_pose.drive_direction = drive_direction;
  simulation_pose.v = this->speed_;
  simulation_pose.vdot = (this->speed_ - old_speed) /
                         ((double)publish_milliseconds_ / 1000) *
                         scale_factor_;  // 1 m/s^2

  simulation_pose.curv = simulation_pose.thetadot /
                         std::max(0.01, simulation_pose.v) /
                         scale_factor_;  // 1/m
