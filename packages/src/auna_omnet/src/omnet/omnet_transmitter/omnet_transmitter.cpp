...
// Create subscribers to get robot data and publisher to send to Artery and
// OMNeT++
OmnetTransmitter::OmnetTransmitter(std::string robot_name)
  ...
  timer = this->create_wall_timer(std::chrono::milliseconds(),
                                  [this]() { cam_callback(); });
...
void OmnetTransmitter::cam_callback() {
  ...
  msg.reference_position.longitude =
      this->longitude_ * scale_factor_ * 10;  // 0.1m
  msg.reference_position.latitude =
      this->latitude_ * scale_factor_ * 10;  // 0.1m
  msg.reference_position.altitude.value =
      this->altitude_ * scale_factor_ * 100;  // 0.01m

  // Adjust for data format
  msg.high_frequency_container.heading.value =
      this->heading_ * 10;  // 0.1 degree
  // Scale car and adjust for data format
  msg.high_frequency_container.speed.value =
      this->speed_ * scale_factor_ * 100;  // 0.01 m/s
  ...
  msg.high_frequency_container.longitudinal_acceleration.value =
      this->acceleration_ * scale_factor_ * 10;  // 0.1m/s^2
  // Scale car data format
  msg.high_frequency_container.curvature.value =
      this->curvature_ / scale_factor_;  // 1/m
  // Adjust for data format
  msg.high_frequency_container.yaw_rate.value =
      this->yaw_rate_ * 100;  // 0.01degree/s

  publisher->publish(msg);
}

// Receive and save the odometry data for speed, acceleration, yaw rate and
// curvature
void OmnetTransmitter::odom_callback(
...
void OmnetTransmitter::pose_callback(
  ...
  this->heading_ =
      this->heading_ -
      360 * floor(this->heading_ / 360);  // get heading in interval [0,360]
}